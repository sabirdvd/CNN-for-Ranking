      ...
      embedding_size, filter_sizes, num_filters, num_hidden, l2_reg_lambda=0.0):
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            self.W = tf.Variable(
                ...
                name="W")
            self.embedded_chars_left = tf.expand_dims(tf.nn.embedding_lookup(self.W, self.input_left), -1)
            self.embedded_chars_right = tf.expand_dims(tf.nn.embedding_lookup(self.W, self.input_right), -1)
